...
/**
 * Token lexer for the Java parser. Heavily based on the public domain grammar file
 * written by <a href="mailto:parrt@jguru.com">Terence Parr </a> et al. See <a
 * href="http://www.antlr.org/resources.html"> http://www.antlr.org/resources.html</a>
 * for more info.
 * 
 * <p>
 * Note that this parser relies on a patched version of ANTLR 2.7.2. It currently won't
    ...
    implements TokenStream, Lexer
{
    ...
    /**
     * Creates a new JavaLexer object. Use {@link #setInputBuffer(Reader)} to set up the
     * input buffer.
     */
    public JavaLexer()
    {
     ...
     *
     * @return <code>true</code> if Javadoc comments will be parsed during processing.
     */
     ...
     *
     * @return <code>true</code> if Javadoc comments should be removed during processing.
     */
     ...
     *
     * @throws IllegalArgumentException if the class is not derived from {@link
    ...
    public void setTokenObjectClass(String clazz)
    {
        ...
        if (!(instance instanceof de.hunsicker.jalopy.language.ExtendedToken))
        {
            throw new IllegalArgumentException(
                "your TokenObject class must extend de.hunsicker.jalopy.language.ExtendedToken");
        }
    }
                ...
                if (!this.removeJavadocComments)
                {
                    ...
                    if (this.parseJavadocComments)
                    {
                        try
                        {
                            String t =
                                new String(
                                    text.getBuffer(), _begin, text.length() - _begin);
                            ...
                            {
                                ExtendedToken token =
                                    new ExtendedToken(JavaTokenTypes.JAVADOC_COMMENT, t);
                    ...
                    else
                    {
                        ...
                        if (t.indexOf('\t') > -1)
                        {
                            t = StringHelper.replace(
                                    t, "\t", StringHelper.repeat(SPACE, getTabSize()));
                        }
...
_loop250: 
            ...
            Token tok =
                new ExtendedToken(
                    JavaTokenTypes.SPECIAL_COMMENT,
                    StringHelper.leftPad(t, t.length() + column));
